...
dbGetQuery(
  con,
  "ALTER TABLE Credential_Non_Dup_STP_CIP4_Cleaning
...
# clean PSI_CREDENTIAL_CIP that don't have 7 characters (i.e., not in the format XX.XXXX)
{
  ...
  ) ## first 2 character do not contain .
...
BGS_Matching_STP_Cdtl_Check_MatchInstAwardYearOnly_orig <- dbGetQuery(
  con,
  qry_BGS_Matching_STP_Credential_PEN_Inst_AwardYearOnly
) ## creates BGS_Matching_STP_Cdtl_Check_MatchInstAwardYearOnly
...
{
  ## get IDs of matches
  ids_exact <- tbl(con, "BGS_Matching_STP_Credential_PEN") %>%
    filter(Final_Consider_A_Match == "Yes") %>%
    distinct(ID) %>%
    collect()
  ids_probable <- tbl(con, "BGS_Matching_STP_Credential_PEN") %>%

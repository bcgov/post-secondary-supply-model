...
intentional_diffs <- list(
  list(table = "t_graduates_r", col = NA,
  ...
  list(table = "trd_data_r", col = "LCIP4_CRED",
       type = "downstream-derived placeholder",
       ticket = "T01",
       note = "Legacy _r carried real/0 values; new _r leaves as NA placeholder -- 02b-1 derives."),
  list(table = "t_appso_data_final_r", col = "LCIP4_CRED",
       type = "downstream-derived placeholder",
       ticket = "T01",
       note = "Legacy _r carried real/0 values; new _r leaves as NA placeholder -- 02b-1 derives."),
  list(table = "t_dacso_data_part_1_stepa_r",
  ...
  list(table = "t_bgs_data_final_r", col = "TOOK_FURTH_ED",
  ...
  list(table = "t_bgs_data_final_r", col = "PEN",
  ...
  list(table = "t_dacso_data_part_1_stepa_r", col = "COCI_PEN",
       type = "encoding improvement (Oracle preserves form)",
       ticket = "T01",
  ...
  list(table = "t_bgs_data_final_r", col = "CURRENT_REGION_PSSM_CODE",
  ...
  list(table = "t_bgs_data_oracle", col = "AGE_GROUP_SRC",
       type = "rename",
       ticket = "T11",
       note = "Oracle source-coded band renamed from AGE_GROUP to AGE_GROUP_SRC."),
  list(table = "t_bgs_data_final_r", col = "INTERNATIONAL",
       type = "new column",
       ticket = "T01",
       note = "Net-new Oracle column not in legacy _r."),
  list(table = "t_trd_data_oracle",
       col = c("GENDER", "LRST_CD", "INTERNATIONAL", "PRGM_ID"),
       type = "new columns",
       ticket = "T01",
       note = "Net-new Oracle columns not in legacy _r."),
  list(table = "t_appso_data_oracle",
       col = c("GENDER", "LRST_CD", "INTERNATIONAL", "PRGM_ID", "PRIVATE_FLAG"),
       type = "new columns",
       ticket = "T01",
       note = "Net-new Oracle columns not in legacy _r."),
  list(table = "t_dacso_data_oracle",
       col = c("INTERNATIONAL", "CREDENTIAL_DERIVED", "COSC_COMPLETED_PRGM_REQ",
               "COSC_CUM_GPA_GROUP", "DATE_OF_BIRTH", "ENRL_END_DATE", "PRGM_ID"),
       type = "new columns",
       ticket = "T01",
       note = "Net-new Oracle columns not in legacy _r.")
)
...
diffs_for <- function(table_name, col_name) {
  Filter(function(d) d$table == table_name && (is.na(d$col)[1] || col_name %in% d$col),
    intentional_diffs)
}

# ---- Snapshot mode: save current dbo *_r as baseline ------------------------
if (SNAPSHOT_MODE) {
  cat("Snapshot mode: saving current dbo *_r as baseline to ", baseline_dir, "\n")
  ...
  cat("Done. Now run R/load-so-survey-oracle.R, then re-run this script without --snapshot-baseline.\n")
  quit(status = 0)
}
...
record_check <- function(pass, label, detail = "") {
  ...
  report_add(glue("- [{status}] {label}"), if (nzchar(detail)) glue("  - {detail}") else "")
}
...
record_warning <- function(label, detail = "") {
  ...
  report_add(glue("- [WARN] {label}"), if (nzchar(detail)) glue("  - {detail}") else "")
}
...
check_keys <- function(name, new_df, old_df, key_col) {
  if (!key_col %in% names(new_df) || !key_col %in% names(old_df)) {
    record_check(FALSE, glue("{name} key ({key_col})"), "key column missing from one side")
    return()
  }
  ...
  detail <- paste(c(
    if (length(in_new_only)) glue("in_new_only: {length(in_new_only)} (e.g. {head(in_new_only, 3)})"),
    if (length(in_old_only)) glue("in_old_only: {length(in_old_only)} (e.g. {head(in_old_only, 3)})")
  ), collapse = " | ")
...
check_cells <- function(name, table_name, new_df, old_df, key_col) {
  if (!key_col %in% names(new_df) || !key_col %in% names(old_df)) {
    record_check(FALSE, glue("{name} cells"), paste("key col", key_col, "missing -- cannot align"))
    return()
  }
  ...
  if (nrow(aligned) == 0) {
    record_check(FALSE, glue("{name} cells"), "no rows aligned by key -- key sets disjoint")
    return()
  }
  ...
  for (col in compare_cols) {
    ...
    if (length(diffs) > 0) next  # skip intentional-diff columns
    new_col <- paste0(col, "_new")
    old_col <- paste0(col, "_old")
    if (!new_col %in% names(aligned) || !old_col %in% names(aligned)) next
    ...
    if (!eq && !identical(class(new_v), class(old_v))) {
      eq <- isTRUE(all.equal(as.character(new_v), as.character(old_v),
    ...
    if (!eq) {
      ...
      diff_mask <- ifelse(is.na(char_new) & is.na(char_old), FALSE,
      ...
      samples <- if (length(diff_idx) > 0) {
        paste(mapply(function(i, n, o) sprintf("key=%s: new=%s | old=%s", format_val(i), format_val(n), format_val(o)),
      ...
      cell_details[[col]] <- glue("{n_diff}/{length(new_v)} rows differ. samples: {samples}")
    }
  }
  pass <- length(cell_fails) == 0
  if (pass) {
    detail <- glue("all {length(compare_cols)} shared non-intentional cols equal ({nrow(aligned)} rows aligned)")
  } else {
    detail <- paste(c(
      glue("mismatched ({length(cell_fails)}): {paste(cell_fails, collapse=', ')}"),
...
format_val <- function(v) {
  ...
  out <- sapply(v, function(x) {
    if (is.na(x)) "NA"
    else if (is.character(x)) paste0("\"", as.character(x), "\"")
    else as.character(x)
  })
  paste(out, collapse = ",")
}
...
for (srv in names(validation_targets)) {
  ...
  new_df <- tryCatch(read_dbo(tgt$new_table),
    error = function(e) { report_add(glue("ERROR reading {tgt$new_table}: {conditionMessage(e)}")); NULL })
  ...
  if (is.null(old_df) && is.null(eda_df)) {
    record_check(FALSE, glue("{toupper(srv)} anchors"),
  ...
  for (anchor_name in c("baseline", "eda")) {
    ...
    if (is.null(anchor_df)) next
    label <- glue("{toupper(srv)} vs {anchor_name}")
    check_schema(label, new_df, anchor_df)
    check_row_count(label, new_df, anchor_df,
      allow_widen = srv == "graduates")
...
if (file.exists(tcr_baseline)) {
  ...
  if (!is.null(new_tcr)) {
    ...
    record_check(eq, "t_cohorts_recoded end-to-end",
      if (eq) "" else "T_Cohorts_Recoded differs pre/post -- intentional-diff composition is wrong somewhere")
  } else {
    record_check(FALSE, "t_cohorts_recoded end-to-end", "could not read new t_cohorts_recoded")
  }
} else {
  report_add("- [SKIP] t_cohorts_recoded end-to-end -- no baseline at ", tcr_baseline,
...
for (d in intentional_diffs) {
  col_str <- if (is.na(d$col)[1]) "(table-level)" else paste(d$col, collapse = ", ")

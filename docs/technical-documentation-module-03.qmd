...
Student records that were transitioned from non-completer to graduate status for cycles (Outcomes 19 and 20) were isolated. This was achieved by isolated records for students whose academic status needed to be corrected, based on confirmed provincial matches ($Has\_STP\_Credential = 'Yes'$). These records are then stratified into a standardized frequency distribution, categorized by demographic age bins and 4-digit program classifications (CIP-4). 

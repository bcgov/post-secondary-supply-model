...
**CIP-4 x Age Frequency Distribution - Near Completers from Survey (column H)**

The "Near-Completer" population (`cosc_grad_status_lgds_cd_group` = '3') is extracted from raw respondent survey reporting cycles, specifically Outcome 19 and Outcome 20. Individual records are binned by categorical age bins where the respondent's age at graduation satisfies boundary conditions specified in the table `AgeGroupLookup`. The resulting table serves as a primary frequency matrix, cross-tabulating age groups with specific institutional credentials and 4-digit Classification of Instructional Programs (CIP) codes. 

Only those credential categories verified for the PSSM 2017-18 model are included in the final output.

**CIP-4 x Age Frequency Distribution - Near Completers Changed to Graduate Status (column I)**

Student records that were transitioned from non-completer to graduate status for cycles (Outcomes 19 and 20) were isolated. This was achieved by identifying records for students whose academic status needed to be corrected, based on confirmed provincial matches (`Has_STP_Credential` = 'Yes'). These records are then stratified into a standardized frequency distribution, categorized by demographic age bins and 4-digit program classifications (CIP-4). 

Only those categories verified for the PSSM 2017-18 model are included in the final output.

**CIP-4 x Age Frequency Distribution - Confirmed Graduate from STP (column K)**

Student records are identified for those who attained a confirmed graduate status (`grad_status_factoring_in_stp` = '1') within the Outcomes 19 and 20 reporting cycles. These records are filtered to include only individuals within the mandatory 17–64 age range and are stratified into discrete age cohorts using a non-equi join with the agegrouplookup table. A frequency table is generated that aggregates counts by institutional credential name and 4-digit program classifications (CIP-4).

Only those categories verified for the PSSM 2017-18 model are included in the final output.

**CIP-4 x Age Frequency Distribution - Confirmed Completers from Survey (column M)**

Student records are identified for individuals attaining confirmed graduate status (`cosc_grad_status_lgds_cd_group` = '1') within the Outcomes 19 and 20 reporting cycles. Observations are restricted to the mandatory 17–64 age range and stratified into discrete age cohorts via a non-equi join with the agegrouplookup table. An initial frequency distribution is generated, aggregating record counts by demographic strata, institutional credential names, and 4-digit program classifications (CIP-4). 

...
WHERE        (STP_Credential.CREDENTIAL_AWARD_DATE NOT IN ('
', '
', '(Unspecified)
')) 
...
FROM       Credential
WHERE      PSI_CREDENTIAL_CATEGORY = '
DEVELOPMENTAL
CREDENTIAL
'
  OR       PSI_CREDENTIAL_CATEGORY = '
OTHER
'
  OR       PSI_CREDENTIAL_CATEGORY = '
NONE
...
WHERE        (CredentialSupVars.ENCRYPTED_TRUE_PEN IS NOT NULL AND CredentialSupVars.ENCRYPTED_TRUE_PEN NOT IN ('
', '
', '(Unspecified)
'))'
...
WHERE       (CredentialSupVars.ENCRYPTED_TRUE_PEN IS NULL OR CredentialSupVars.ENCRYPTED_TRUE_PEN IN ('
', '
', '(Unspecified)
'))
AND         (CredentialSupVars.PSI_CODE IS NOT NULL AND CredentialSupVars.PSI_CODE NOT IN ('
', '
', '(Unspecified)
')) 
AND         (CredentialSupVars.PSI_STUDENT_NUMBER IS NOT NULL AND CredentialSupVars.PSI_STUDENT_NUMBER NOT IN ('
', '
', '(Unspecified)
'));'
...
SET    PSI_AWARD_SCHOOL_YEAR = CASE
	WHEN (Month(Credential.CREDENTIAL_AWARD_DATE_D) >= 9) THEN LTrim(Str(Year(Credential.CREDENTIAL_AWARD_DATE_D))) + ' /
  ' + LTrim(Str(Year(Credential.CREDENTIAL_AWARD_DATE_D)+1))
	ELSE LTrim(Str(Year(Credential.CREDENTIAL_AWARD_DATE_D)-1)) + ' /
  ' + LTrim(Str(Year(Credential.CREDENTIAL_AWARD_DATE_D)))
	END
...
WHERE     (Credential.PSI_GENDER_cleaned IN ('
', '
', '(Unspecified)
') 
OR         Credential.PSI_GENDER_cleaned IS NULL) 
AND       (STP_Enrolment.PSI_GENDER IN ('
Female
', '
Male
', '
...
FROM      Credential_Non_Dup
WHERE     psi_gender_cleaned IN ('
', '
', '(Unspecified)
') OR psi_gender_cleaned IS NULL;'
...
Diverse
'
WHERE PSI_GENDER_CLEANED NOT IN('
Female
','
Male
') OR PSI_GENDER_CLEANED IS NULL;
...
WHERE        (a.PSI_STUDENT_NUMBER IN
                             (SELECT        PSI_STUDENT_NUMBER
                               ...
                               HAVING         (COUNT(PSI_STUDENT_NUMBER) > 1) AND ((ENCRYPTED_TRUE_PEN IS NULL) OR (ENCRYPTED_TRUE_PEN IN ('
', '
', '(Unspecified)
')))))'
...
qry08b_Rank_non_multi_cred <- '
UPDATE Credential_Non_Dup
SET Credential_Non_Dup.Highest_Cred_by_Date = '
Yes
', 
...
FROM    Credential_Non_Dup
WHERE   (AGE_AT_GRAD IS NULL) AND (Highest_Cred_by_Date = '
Yes
');
'
...
FROM         Credential_Non_Dup
WHERE     (AGE_GROUP_AT_GRAD IS NOT NULL) AND (Highest_Cred_by_Date = '
Yes
')
...
SELECT    Credential_Non_Dup.id, 
          ...
          Credential_Non_Dup.AGE_AT_GRAD, 
          Credential_Non_Dup.AGE_GROUP_AT_GRAD, 
...
ON Credential_Non_Dup.PSI_CREDENTIAL_CATEGORY = CredentialRank.PSI_CREDENTIAL_CATEGORY
INNER JOIN tblCredential_HighestRank 
ON Credential_Non_Dup.CONCATENATED_ID = tblCredential_HighestRank.CONCATENATED_ID
WHERE Credential_Non_Dup.CREDENTIAL_AWARD_DATE_D > tblCredential_HighestRank.CREDENTIAL_AWARD_DATE_D;'


qry18b_ExtrLaterAwarded <- "
SELECT 
    tblCredential_LaterAwarded.LID, 
...
WHERE 
    (tblCredential_LaterAwarded.PSI_CREDENTIAL_CATEGORY = 'APPRENTICESHIP' 
...
SELECT 
  qry18b_ExtrLaterAwarded.CONCATENATED_ID,
  Max(qry18b_ExtrLaterAwarded.LATER_AWARD_DATE) AS MaxOfLATER_AWARD_DATE
INTO qry18c_ExtrLaterAwarded
FROM qry18b_ExtrLaterAwarded
GROUP BY qry18b_ExtrLaterAwarded.CONCATENATED_ID;'

qry18d_ExtrLaterAwarded <- '
SELECT DISTINCT 
  Min(qry18b_ExtrLaterAwarded.LID) AS LID, 
  qry18b_ExtrLaterAwarded.HID, 
  qry18b_ExtrLaterAwarded.CONCATENATED_ID, 
  qry18b_ExtrLaterAwarded.LATER_AWARD_DATE, 
  qry18b_ExtrLaterAwarded.PSI_AWARD_SCHOOL_YEAR 
INTO tblCredential_DelayEffect
FROM qry18b_ExtrLaterAwarded 
INNER JOIN qry18c_ExtrLaterAwarded 
  ON (qry18b_ExtrLaterAwarded.LATER_AWARD_DATE = qry18c_ExtrLaterAwarded.MaxOfLATER_AWARD_DATE) 
  AND (qry18b_ExtrLaterAwarded.CONCATENATED_ID = qry18c_ExtrLaterAwarded.CONCATENATED_ID)
GROUP BY 
  qry18b_ExtrLaterAwarded.HID, 
  qry18b_ExtrLaterAwarded.CONCATENATED_ID, 
  qry18b_ExtrLaterAwarded.LATER_AWARD_DATE, 
...
HAVING        (tblCredential_HighestRank.PSI_CREDENTIAL_CATEGORY <> '
APPRENTICESHIP
')
...
FROM            tblCredential_HighestRank INNER JOIN
                         ...
                         Credential_Non_Dup ON tblCredential_HighestRank.id = Credential_Non_Dup.id
WHERE        (Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
09
' AND Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
10
')
GROUP BY AgeGroupLookup.AgeGroup, tblCredential_HighestRank.PSI_CREDENTIAL_CATEGORY, tblCredential_HighestRank.PSI_AWARD_SCHOOL_YEAR_DELAYED
HAVING        (tblCredential_HighestRank.PSI_CREDENTIAL_CATEGORY <> '
...
FROM            tblCredential_HighestRank INNER JOIN
                         AgeGroupLookup ON tblCredential_HighestRank.AGE_GROUP_AT_GRAD = AgeGroupLookup.AgeIndex
WHERE        (tblCredential_HighestRank.PSI_VISA_STATUS = '
DOMESTIC
') OR
...
09
' AND 
                         Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
10
') OR
                         (tblCredential_HighestRank.PSI_VISA_STATUS IS NULL) AND (Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
09
' AND 
                         Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
10
...
WHERE        (tblCredential_HighestRank.PSI_VISA_STATUS = '
DOMESTIC
') AND (tblCredential_HighestRank.RESEARCH_UNIVERSITY = 1) AND 
                         (tblCredential_HighestRank.OUTCOMES_CRED <> '
DACSO
') OR
...
DACSO
') OR
                         (tblCredential_HighestRank.PSI_VISA_STATUS = '
DOMESTIC
') AND (tblCredential_HighestRank.RESEARCH_UNIVERSITY IS NULL) OR
...
') OR
                         (tblCredential_HighestRank.PSI_VISA_STATUS = '
DOMESTIC
') AND (tblCredential_HighestRank.RESEARCH_UNIVERSITY IS NULL) AND 
                         (Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
09
') AND (Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
10
...
10
') AND 
                         (Credential_Non_Dup.RESEARCH_UNIVERSITY = 1) AND (Credential_Non_Dup.OUTCOMES_CRED <> '
DACSO
') OR
                         (tblCredential_HighestRank.PSI_VISA_STATUS IS NULL) AND (Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
09
...
') AND 
                         (Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
10
') AND (AgeGroupLookup.AgeIndex <> 1) AND (AgeGroupLookup.AgeIndex <> 9) OR
                         (tblCredential_HighestRank.PSI_VISA_STATUS IS NULL) AND (tblCredential_HighestRank.RESEARCH_UNIVERSITY = 1) AND 
                         (tblCredential_HighestRank.OUTCOMES_CRED <> '
DACSO
') AND (Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
...
10
') AND (AgeGroupLookup.AgeIndex <> 1) 
                         AND (AgeGroupLookup.AgeIndex <> 9) AND (CredentialSupVars.PSI_VISA_STATUS = '
DOMESTIC
') AND (Credential_Non_Dup.RESEARCH_UNIVERSITY = 1) AND 
                         (Credential_Non_Dup.OUTCOMES_CRED <> '
DACSO
') OR
                         (Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
...
') AND (AgeGroupLookup.AgeIndex <> 1) 
                         AND (AgeGroupLookup.AgeIndex <> 9) AND (CredentialSupVars.PSI_VISA_STATUS = '
DOMESTIC
') AND (Credential_Non_Dup.RESEARCH_UNIVERSITY IS NULL) OR
                         (Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
09
') AND (Credential_Non_Dup.FINAL_CIP_CLUSTER_CODE <> '
10
...
HAVING        (Credential_Non_Dup.PSI_CREDENTIAL_CATEGORY <> '
APPRENTICESHIP
')
...
HAVING        (RESEARCH_UNIVERSITY = 1) AND (OUTCOMES_CRED = '
DACSO
') AND (PSI_AWARD_SCHOOL_YEAR_DELAYED = '
2018 / 2019
')
...
WHERE     (Credential_Non_Dup_Exclude_LatestYr.Highest_Cred_by_Rank = '
Yes
')'
